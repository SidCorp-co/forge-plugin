...
/* The second non-review kind in this log, and the printer's one distinction that a review has not
   got: a refusal — the answer came back outside the ladder — is the row's own word, where `failed` is
   the gateway's. */
test("a proposal row reads as the issue it read and the complexity proposed, not as a review with no files", () => {
  const row = {
    kind: "complexity", at: "2026-09-22T10:00:00.000Z", model: "cx/gpt-5.6-sol", ms: 4_000,
    ok: true, key: "ISS-457", proposed: "s", confidence: 0.8, why: "one file, one enum",
    run: "iss-2161-x", runFrom: "asked",
  };
  assert.equal(logLine(row, false),
    "2026-09-22T10:00:00.000Z  cx/gpt-5.6-sol  4s  complexity ISS-457 -> s  confidence 0.8  by iss-2161-x (asked)");
  ...
  assert.equal(logLine({ ...row, confidence: null }, false),
    "2026-09-22T10:00:00.000Z  cx/gpt-5.6-sol  4s  complexity ISS-457 -> s  by iss-2161-x (asked)",
    "a confidence the model did not give is not printed as one");
  assert.match(logLine({ ...row, proposed: undefined, refused: "`huge` is no complexity" }, false),
    /complexity ISS-457 {2}refused: `huge` is no complexity/u, "an answer outside the ladder is the row's own refusal");
  assert.match(logLine({ ...row, ok: false, error: "the gateway streamed no text at all" }, false),
