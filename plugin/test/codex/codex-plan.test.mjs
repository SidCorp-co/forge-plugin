...
test("the system prompt carries a version and a digest, and the recheck clause is only a recheck's", () => {
  ...
  assert.equal(promptMark(pass).v, 5, "pinned by hand, so a bump nobody meant fails here");
...
test("a stored consult reading is the before window, scored as it was at the mark, on every checkout", () => {
  ...
  assert.deepEqual(pinned.overlap, { shared: 50, recent: 100, untilReadable: 0, untilDisjoint: 50 }, "answered 101-200 against 151-250");
