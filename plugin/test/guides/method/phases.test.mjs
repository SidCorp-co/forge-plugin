...
test("Phase 3 says where a payload file is written, in every flow and under either reviewer", () => {
  const beats = [
    ...
    ["what answers where it is not", "in every other case under a directory the run makes for itself in the system's temporary one"],
    ["why an inherited directory is not the run's own", "so the host's scratchpad is the whole wave's"],
