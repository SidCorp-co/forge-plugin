...
test("a merge mark is printed on the candidate and moves neither its place nor its score", async () => {
  ...
  assert.match(line, /^ {2}merged merge mark set 2026-09-06T19:09:19\.786Z, \d+ day\(s\) ago, while the status reads reopen and the issue has been reopened 2 time\(s\)$/u);
...
test("a batch member with a merge mark says so on its own line and in the json", async () => {
  ...
  assert.equal(run.status, 0, run.stderr);
  assert.match(run.stdout, /^ {2}\+ ISS-2 .* · merge mark set 2026-09-06T19:09:19\.786Z, \d+ day\(s\) ago$/mu);
