...
import { complexitiesOf, costFor, isWarm, lastLanded, measuredRuns, owesRestart } from "../../src/rank/cost.mjs";
...
/* A dash where a corpus exists would read as no corpus at all, so a complexity nothing landed in
   says what it fell back to instead of going quiet. */
test("a complexity no past run landed in falls back to every run and says which", () => {
  const runs = [{ key: "ISS-11", minutes: 40 }, { key: "ISS-12", minutes: 60 }, { key: "ISS-13", minutes: 80 }];
  const held = new Map([["ISS-11", "unset"], ["ISS-12", "unset"], ["ISS-13", "m"]]);
  const own = costFor("unset", runs, held);
  assert.deepEqual(own, { minutes: 50, over: 2, complexity: "unset" });
  const none = costFor("xs", runs, held);
  assert.equal(none.minutes, 60, "the median of all three");
  assert.equal(none.over, 3);
  assert.equal(none.complexity, null, "and the one it could not answer at is not claimed");
});

test("the complexity of a past run is read off the browse projection alone", () => {
  const held = complexitiesOf([{ issueId: "ISS-11", complexity: "m" }, { issueId: "ISS-12", complexity: null }]);
  assert.equal(held.get("ISS-11"), "m");
  assert.equal(held.get("ISS-12"), "unset", "no body was read, and no body is a source of one");
});
