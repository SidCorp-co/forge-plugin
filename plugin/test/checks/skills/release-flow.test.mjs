...
/* Matched against the text as one line, so a rule stays found when a later edit rewraps its
   paragraph: a pattern that breaks on a line break is testing the margin, not the rule. */
...
const RULES = [
  ...
  ["12 · both green: say which two readings that was", /Say which two readings that was/u],
  ["12 · health red: look at what the build was given", /look at what it depends on, what it was given, and what it says in its own log/u],
  ["12 · identity red: the application is not the repair", /Repairing the application here fixes nothing, twice/u],
  ["12 · both red: establish arrival before behaviour", /Establish that before diagnosing anything about behaviour/u],
  ...
  ["15 · the repair stays in the run already open", /all within the run you are already in/u],
  ["16 · the two limits come from the job's policy", /are the job's own policy/u],
  ...
  ["19 · a failed production reading never closes", /A failed production observation ends the success path, and never in a close/u],
  ["19 · repair inside the job is the ordinary case", /that is the ordinary case, not an exception to this/u],
...
test("no rule's pattern is loose enough to match the issue method instead", () => {
  const other = flat(servedBody("issue-flow", undefined, DEFAULT));
  for (const [said, pattern] of RULES) {
...
test("the method offers no way back, under either flow", () => {
  for (const flow of [DEFAULT, SCREEN]) {
    assert.doesNotMatch(flat(body(flow)), /\brolls? back\b|\brollback\b|\brolling back\b/iu,
...
test("no sentence of the method prescribes a language to write in", () => {
  assert.doesNotMatch(flat(body()), /\bin (?:English|Vietnamese)\b|tiếng Việt/iu);
});
