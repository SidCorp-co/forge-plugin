...
test("the citation check has one home, and the verbs reach it rather than spelling it", () => {
  const commands = source("src/commands.mjs");
  assert.ok(commands.includes('from "./spec/checked.mjs"'), "the plan verb spends the shared step");
  assert.ok(!commands.includes("citationRefusal"), "and does not assemble the refusal itself");
  assert.ok(source("src/flow/record.mjs").includes('from "../spec/checked.mjs"'), "and so does record criteria");
});
