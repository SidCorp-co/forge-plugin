...
Why: an agent finishing a task reaches for "save what I learned" as a closing ritual, and the corpus
fills with rows nobody reads, burying the two that mattered. A memory row is *project knowledge*; a
...
A skill edit answers three things instead: the category, whether a check in the plugin could enforce
it, and what it displaces. A sentence the skill already says is refused outright, since two
authorities for one rule diverge the first time someone corrects the copy they found.

How to write through it: use `Write` or `Edit`. The shell route is closed rather than approximated,
