...
const KNOWN = [...CLAUSES.map((one) => one.flag), "to", "target"];
...
/** `forge record merged`: one flag per clause of the note, and `--undo` the one route back. */
export const recordMerged = async (reference, argv, { next, patch } = {}) => {
  const given = flags(argv, "record merged", ["--undo"]);
  for (const one of Object.keys(given)) {
    if (one !== "undo" && !KNOWN.includes(one)) {
      refuse(`record merged takes no --${one}. Flags: ${KNOWN.map((two) => `--${two}`).join(" ")} --undo`);
    }
  }
