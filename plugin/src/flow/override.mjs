...
import { OVERRIDE, ownsField, writeField } from "../tracker/field-write.mjs";
import { AMBIGUOUS } from "../tracker/rpc.mjs";
import { issueOf, post, render } from "./record/record.mjs";

const PAIR = /^([A-Za-z][\w.]*)=([\s\S]*)$/u;
...
/* The status on which the tracker reads any comment as the reporter's answer and puts the issue back to `open` (ISS-429). A record whose write moves a status is the one thing an override is for the opposite of, so the field set is refused here rather than followed by a move nobody asked for. */
const ASKED = "needs_info";
...
export const correctionFor = async (documentId, ref, moved, why, { done = true } = {}) => {
  ...
  const answer = await post(documentId, body, ref, undefined, null, true);
  if (!answer?.refused) return;
  /* A dropped write may have landed, so neither route may say the record is not there: told it is, a
     run posts the body again or overrides again, and the page carries the same claim twice. */
  const unknown = answer.refused.includes(AMBIGUOUS);
  const said = unknown ? "may or may not have gone up" : "did not go up";
  const opened = done
  ...
  refuse(done
    ? `${opened}\nNothing on the page now says a run set it by hand. Put this body up as it `
      + `stands:\n\nforge comment ${documentId} - <<'FORGE_CORRECTION'\n${body}\nFORGE_CORRECTION`
    : `${opened}\nSo there is no move to correct and no record claiming one. Run the same override again.`);
};

export const setPair = (given) => {
  const found = PAIR.exec(String(given ?? ""));
  if (!found) {
    refuse(`--set takes \`<field>=<value>\`, not \`${given}\`: the field is the tracker's own name `
      + "for it, and everything after the first `=` is the value.");
  }
...
export const overrideField = async (reference, given, why, { next, patch } = {}) => {
  ...
  const back = await writeField(documentId, field, value, { ref: reference, next, patch, refuse, row: OVERRIDE });
