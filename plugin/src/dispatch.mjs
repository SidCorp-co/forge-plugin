...
/* One link on PATH is one copy for the machine, so a call arriving through it chooses its copy here
   by working directory and by verb, with argv[1] set to the real path a main-module guard compares
   against. The imports are node builtins and the choosers: a graph reaching what this survives
   survives nothing. */
...
const note = flowNote(chosen, rest[0]);
...
try {
  ...
  if (chosen.tree) {
    console.error(`\n${join(chosen.tree.dir, "bin", "forge")} runs this checkout's own copy instead.`);
  }
