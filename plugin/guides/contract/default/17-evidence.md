### Evidence — what a payload is trusted to say

**What evidence a criterion needs is its author's to name and the reviewer's to judge.** This
contract checks presence and the commit, never truth. A payload is machine data: it travels in a form
the project's prose rewrite leaves alone, and its field names are the flags the verb took.
...
record is corrected, never removed. Name the binary by path when a verdict says *the installed copy*.
