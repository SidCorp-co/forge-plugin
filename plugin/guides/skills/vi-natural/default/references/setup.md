...
Key and defaults live in `~/.config/vi-natural/config.json` (`chmod 600`) and nowhere else — the
environment is not read. Precedence: flag → `.vi-glossary.json` → config file.
