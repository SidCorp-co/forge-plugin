...
consult log, `forge stats eval` for the issue-flow corpus. Both write nothing. Take their `--json`
where a figure has to be quoted exactly, and keep the screen form for the shape of the comparison.
...
`forge new --into`, marked `Size: fix.`, carrying the two values, the counts and the attribution. A
