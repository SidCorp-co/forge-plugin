...
| `awaiting_release` | the evidence is here to be judged, and you can see it now | 6, 7 Ship, the close |
...
Phase 7, so the judging that earns `awaiting_release` happens while the issue is still
`in_progress`, and both statuses move on records written before the mark.
