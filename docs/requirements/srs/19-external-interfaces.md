...
- **AC-19-8-134** · Rev: 2 · Proof: plugin/test/stats/eval/ruled.test.mjs "a ruling call whose text names no id takes the one its run's claims printed under a source naming a run"
  WHERE a ruling call's run is named, by an id its own command grants or else, for a command that
  assigns no id and takes none back, by the one id its run's claims printed as given or found in its
  own tree, the CLI SHALL pair that call with the one entry of the consult log that run wrote among
  those the call reaches, SHALL NOT pair it with an entry another run wrote, SHALL take as a run's
  only an id the log says the run was given or found in its own tree, SHALL give an entry that run
  wrote to that call over any call naming no run, SHALL name no run for a call whose run's claims
  printed two ids, and SHALL otherwise pair a call or an entry that names no run as it would with no
...
- **AC-19-8-115** · Rev: 1 · Proof: tools/test/run/release/run-note.test.mjs "a version step run from a subdirectory names the path it joined, not just the value it read"
...
- **AC-19-8-116** · Rev: 1 · Proof: tools/test/run/release/run-released-version.test.mjs "a ship resumed with --from 7 from a subdirectory refuses a version it cannot read, rather than pushing an untagged release to the remote and saying nothing"
...
- **AC-19-8-117** · Rev: 1 · Proof: tools/test/run/release/run-released-version.test.mjs "a release's last step refuses a version it cannot read from a subdirectory, even where the remote already carries the real tag"
...
- **AC-19-8-118** · Rev: 1 · Proof: tools/test/run/release/run-released-version.test.mjs "a ship resumed with --from 7 from a subdirectory refuses a version it cannot read, rather than pushing an untagged release to the remote and saying nothing"
