...
| **C-07** The tracker's comment list serves a row window and names the cursor for the window behind it. | 2 | A thread longer than one window costs a request per window, and a walk that stops before the tracker calls the read complete holds a prefix whose missing rows it cannot name: what the page earns it earns, and a shortfall may name a record the walk never reached (ISS-17, ISS-131). |
