...
| **C-05** The tracker refuses no stale write yet. | 1 | The lease is advisory: two runs that both find no lease both take one. A project running more than one agent needs the tracker's refusal first (ISS-7). |
