...
| **M-02** Raw tracker transitions per run, against transitions made by the verb. | each run's own records on the tracker | falling to none |
| **M-03** Tracker calls made by hand to finish one issue. | the same records | falling |
| **M-04** Payloads a run had to invent at the keyboard because no shape existed. | the same records | none |
