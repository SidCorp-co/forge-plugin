# `stats runs` — what opens the last phases, and the act nobody commands

The rows themselves, and the classifier they are read by: [stats — the rows](stats-rows.md). This
page is the half of that reading which is about what happens once a change is built.
