...
A model asked for a number between nought and one gives one; nothing made it honest. So the figure is
printed as given, sorts a list of proposals if a reader wants one sorted, and gates nothing: no refusal,
no write and no silence turns on it crossing a value. A design that thresholds a self-reported
confidence has built a coin it cannot see the weighting of.

## No date travels

The state sent is the key, the title, the category and the body, and every ISO date in the body is
one marker before it goes. A literal reader turns "filed 2026-09-06" into age, and age is already the
largest single term in this project's rank (ISS-2157) — a question meant to read the work would be
reading the calendar. The role says what the marker stands for, so the model does not guess.

## Why a model of this family is not refused here

C-09 refuses a same-family reviewer because a second opinion on one's own work from one's own family
is an echo. This verb reviews nothing: the model is asked about an issue somebody else wrote, and its
