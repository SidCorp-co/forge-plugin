# The kinds, and the sections each one's body owes

What a body is refused for, the routes past a refusal, and how a filing is ranked are
[`new`](new.md)'s. This is the set itself, and where it came from.
...
`--category` is required of a filing. It decides which sections the body is read against and it is
the tracker's own field, and prose answers neither: the same headings carry a bug and a feature, and
...
because a reading filed as a feature reads on every list as work somebody still has to do. The value
has to reach the field for a reader to filter it off.
...
neither on the way in or on the way out. It used to: `--kind` stood for `category`, `--size
trivial|fix|feature` for `complexity xs|s|m|l|xl`, and a read renamed both back again. Two
vocabularies for one field is a translation table to keep, a value that reads back under a name the
tracker never held, and an agent that has to learn which of the two a given screen speaks. The three
