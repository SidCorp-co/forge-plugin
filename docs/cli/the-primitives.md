...
## The one that is a predicate rather than a pattern

`wantsHelp` was the same finding again and its own topic:
[the help predicate](the-help-predicate.md), where a pattern's question — what does this match — is
not the one a predicate is judged on.
