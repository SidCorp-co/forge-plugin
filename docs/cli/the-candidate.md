# the candidate — the commit a landing builds, and what a reading taken over it is a fact about

One landing may carry several branches, and the commit it builds from them is what every reading of
that landing is filed under. What a checkpoint holds, whose turn each state names and which routes
the table walks: [`the-checkpoint.md`](the-checkpoint.md).
