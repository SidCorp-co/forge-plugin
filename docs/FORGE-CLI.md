...
The tracker's write-time rules are carried in the binary and print on `-h` only. A guide costs 3–6 KB
to fetch, and paying that lecture for a typo cost more bytes than `forge -h` itself.
