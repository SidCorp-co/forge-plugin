...
| [stats — the condition](cli/stats-the-condition.md) | Why a compaction, an API error and a human prompt are counted apart from a refusal this plugin wrote, why each prints as two counts, why the runs a human prompt showed up inside are named rather than rated, and why none of the three is folded into the angle set. |
