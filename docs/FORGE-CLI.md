...
| [stats — the reading](cli/stats-the-reading.md) | Why the daily page is read by three roles rather than one, why a model cites a figure by its key and never restates it, why the call is not a consult, and where the closed count and its minutes come from. |
