...
| [`next` weights](cli/next-weights.md) | Why the complexity field is the one source, why age has no ceiling and an order held by it is the answer, and what an issue nobody sized is worth. |
...
| [one transport](cli/one-transport.md) | Why the route each capability takes is a declared table, what a capability with no route does instead, why a write announces itself here, and why what a write declined is read off the answer rather than out of a row's projection. |
