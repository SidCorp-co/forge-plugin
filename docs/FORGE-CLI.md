...
| [the name join](cli/the-name-join.md) | Why the comparison between the names a row shaper asks the tracker for and the names the tracker answers is a verb somebody runs rather than a step of the gate, why the names a shaper reads are recorded from the shaper rather than read off its source, and why two of the row's columns are excluded by name and counted rather than named. |
