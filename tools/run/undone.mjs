...
export const cutOf = (tree, branch, was) => {
  ...
  const logged = lines(said(tree, ["log", "-g", "--format=%H", `refs/heads/${branch}`, "--"]));
  if (!logged.length) return { why: `refs/heads/${branch} holds no reflog in this repository` };
  const at = gitOut(["merge-base", logged.at(-1), was], tree);
...
/* Each line of the file at `was` as the commit that wrote it and its line number in that commit. */
const blameOf = (tree, was, file) => {
  const out = [];
  let at = null;
  for (const line of said(tree, ["blame", "--porcelain", was, "--", file]).split("\n")) {
 ...
 *  fewer than half come back anywhere in the change, which is what a move does, and what the hunk
 *  replaced comes back in this file, which a rewrite into new content does not do and a stale copy
 *  always does. */
export const takesBack = ({ standing, removed, addedAnywhere, addedHere, replaced }) => {
  ...
  return back * 2 < wordy.length && holdsAll(addedHere, replaced);
};

const removedAt = (hunks) =>
  new Set(hunks.flatMap((one) => Array.from({ length: one.oldCount }, (_, at) => one.oldStart + at)));

const fileTakesBack = (tree, { was, head, file, landed, addedAnywhere }) => {
...
export const undoneBy = (tree, { was, head, branch }) => {
  ...
  for (const [file, shas] of touchedBy(tree, cut.at, was, head)) {
    ...
    for (const sha of fileTakesBack(tree, { was, head, file, landed: open, addedAnywhere })) {
