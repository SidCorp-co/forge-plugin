...
export const reviewBody = ({ tree, from, to, volume, self }) => {
  ...
  return [
    ...
    "  name, and the hand reading covers its ground. Then the hand reading, over what the list does",
    "  not raise. Every finding says which of the two raised it, and the confirmation cites the",
    "  reading, so the next batch's fold can count what each found.",
    "- That reading is read, never applied. `simplify` invoked as a pass writes to the tree it",
    "  reviews, so it is given the diff and answers with findings. `git status --porcelain` is read",
    "  after it and before the one landing, and anything it wrote is reverted rather than committed.",
