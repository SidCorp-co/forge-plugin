...
import { changeRead, stillReads } from "./candidate.mjs";
  ...
  await perMemberOwed(at, async (member) => {
    ...
    if (markedCommit(comments ?? []) === landed) {
      ...
      /* git's reading of the judged head against the candidate the change landed as, which is what the
         ...
         verdicts' to answer for, so the clause reads it as the chain did (ISS-2516). Nor is a path the
         landed head's own generators write back, which the chain carried on the same reading (ISS-1421). */
      const over = landing.candidate ?? landed;
      const moved = changeRead(root, judged, over, landing.files)?.moved
