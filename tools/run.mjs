...
const usage = () => [
  ...
  "                          holds its lease under, the one directory its scratch belongs in, and",
  "                          the form a probe needing live data borrows this machine's credentials",
  "                          through, read-only, instead of copying them into that directory.",
  ...
  "                          work no record cites or while a file in the scratch holds a copy of",
  "                          one of this machine's credentials, and says what it left and why",
  `  ${sig("ship")}`,
  ...
  "                          below. A candidate the gate refuses is searched where the project's",
  "                          `redBatch` is attribute-then-split, the default: a member every failing",
  "                          case's paths name alone goes back to its builder and the rest are gated",
  "                          once, and otherwise the suspects are halved, each half gated on the same",
  "                          pin, so the members left green land as one candidate and one release.",
  "                          `redBatch` one-by-one lands each branch alone instead, against the base",
  "                          as it moves. Where the landing is is each issue's checkpoint, so a",
  "                          second run finishes what is owed and needs no step number. Naming no",
  "                          issue takes every one this project left ready,",
