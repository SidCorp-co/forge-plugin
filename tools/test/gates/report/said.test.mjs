...
import { ledgerSaid, readsSaid, stepSaid, verdictSaid } from "../../../gates/report/said.mjs";
import { fileSeconds, fileTimesPath } from "../../../gates/timing.mjs";
...
test("a run whose record holds nothing says so, of its steps and of its test files alike", () => {
  ...
  try {
    ...
    assert.match(stdout, /gate verdict: pass — \d+ of 14 step\(s\) in \d+s, test:tree /u, stdout);
    assert.ok(stdout.includes(`, test:tree ${NAMED.length} of ${NAMED.length} file(s), `), stdout);
...
test("a step every one of whose files the record answers for prints its block and is never spent", () => {
  ...
  try {
    ...
    assert.ok(stdout.includes(`=== reads: test:tree — ${NAMED.length} of ${NAMED.length} test file(s)`), stdout);
  } finally {
    rmSync(at, { recursive: true, force: true });
  }
});
